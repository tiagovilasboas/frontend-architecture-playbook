...
export function recommendationsForTest(answers: TestAnswers): string[] {
  const { projectType, teamSize, techLevel, priority, hasIntegrations } =
    answers;
  ...
  if (teamSize === "large") {
    ...
    scores["clean-architecture"] = {
      ...
      reason:
        scores["clean-architecture"]?.reason || "Organização para time grande",
    };
    scores["hexagonal"] = {
  ...
  if (techLevel === "senior") {
    ...
    scores["hexagonal"] = {
      score: (scores["hexagonal"]?.score || 5) + 2,
      reason: scores["hexagonal"]?.reason || "Time domina abstrações complexas",
    };
    scores["event-sourcing"] = {
    ...
    scores["cqrs"] = {
      ...
      reason:
        scores["cqrs"]?.reason || "Time pode implementar separação read/write",
    };
    scores["microservices-frontend"] = {
  ...
  if (priority === "maintainability") {
    scores["clean-architecture"] = {
      ...
      reason: scores["clean-architecture"]?.reason || "Máxima manutenibilidade",
    };
    scores["component-driven"] = {
  ...
  if (priority === "scalability") {
    if (teamSize === "large") {
      scores["micro-frontends"] = {
        ...
        reason:
          scores["micro-frontends"]?.reason || "Máxima escalabilidade de times",
      };
      scores["microservices-frontend"] = {
    ...
    scores["monorepo"] = {
      score: (scores["monorepo"]?.score || 5) + 3,
      reason: scores["monorepo"]?.reason || "Facilita escalabilidade de código",
    };
    scores["clean-architecture"] = {
      score: (scores["clean-architecture"]?.score || 6) + 2,
      reason:
    ...
    scores["pwa"] = {
      ...
      reason:
        scores["pwa"]?.reason || "Escalabilidade de plataforma (web → mobile)",
    };
