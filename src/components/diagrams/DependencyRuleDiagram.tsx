...
export default function DependencyRuleDiagram({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const padding = verticalLayout
        ? 16
        : Math.max(24, Math.min(48, w * 0.04));
      ...
      const labelFont = verticalLayout
        ? '600 14px Inter, sans-serif'
        : '600 18px Inter, sans-serif';
      const subFont = verticalLayout
      ...
      if (verticalLayout) {
        ...
        const centerY = h / 2;
        const startX =
          padding + (w - padding * 2 - 4 * boxW - totalGaps) / 2 + boxW / 2;
        const textPadding = 10;
        const maxTextW = Math.max(40, boxW - textPadding * 2);
