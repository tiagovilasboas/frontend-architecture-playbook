...
const CONFIG_OPTIONS = [
  ...
  { value: 'small-cars', label: 'Carros Pequenos (Cima)', config: SMALL_CARS_TOP_CONFIG },
  { value: 'large-cars', label: 'Carros Grandes (Baixo)', config: LARGE_CARS_BOTTOM_CONFIG },
  { value: 'high-position', label: 'Posição Alta', config: HIGH_POSITION_CONFIG },
  { value: 'high-traffic', label: 'Trânsito Intenso', config: HIGH_TRAFFIC_CONFIG },
...
export default function HighwayConfigDemo() {
  ...
  const currentConfig = CONFIG_OPTIONS.find(opt => opt.value === selectedConfig)?.config || SIMPLE_HIGHWAY_CONFIG;
  
  const getConfigInfo = (config: HighwayConfig) => {
    const totalCars = config.lanes.reduce((total: number, lane) => 
      total + lane.sublanes.reduce((laneTotal: number, sublane) => laneTotal + sublane.numCars, 0), 0
    );
    
    const totalLanes = config.lanes.reduce((total: number, lane) => 
      total + lane.sublanes.length, 0
    );
    
    const avgSpeed = config.lanes.reduce((total: number, lane) => 
      total + lane.sublanes.reduce((laneTotal: number, sublane) => 
        laneTotal + (sublane.maxSpeed + sublane.minSpeed) / 2, 0
      ), 0
    ) / totalLanes;
      ...
      <Stack gap="lg">
        ...
        <Group>
          <Select
            ...
            data={CONFIG_OPTIONS.map(opt => ({ value: opt.value, label: opt.label }))}
            w={300}
          />
          ...
          <Group gap="xs">
            ...
            <Badge variant="light">
              Vel. média: {info.avgSpeed}
            </Badge>
          </Group>
        </Group>
        
        <Text c="dimmed" size="sm">
          Teste diferentes configurações de rodovia para ver como a abstração permite 
          customizar facilmente posições, escalas e comportamentos dos veículos.
        </Text>
        ...
        <Stack gap="xs">
          <Text size="sm" fw={500}>Configuração Atual:</Text>
