...
import {
  ...
  CodeBlock,
  DiagramBlock,
} from '../components/playbook-theme';
import {
  IconStack2,
  IconGitBranch,
  IconShield,
...
const TOC_ITEMS = [
  { number: '01', title: 'Princípios Arquiteturais', id: '01-principios' },
  { number: '02', title: 'Estrutura de Projeto', id: '02-estrutura' },
  { number: '03', title: 'Gerenciamento de Estado', id: '03-estado' },
  { number: '04', title: 'Design System & Tokens', id: '04-design-system' },
  { number: '05', title: 'Performance & Otimização', id: '05-performance' },
  { number: '06', title: 'Testing Strategy', id: '06-testing' },
  { number: '07', title: 'CI/CD & Deploy', id: '07-cicd' },
  { number: '08', title: 'Observabilidade', id: '08-observabilidade' },
];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <PlaybookHero
        ...
        tags={['React', 'TypeScript', 'Design Systems', 'Performance', 'Testing', 'CI/CD']}
      />

      <TableOfContents title="Índice" items={TOC_ITEMS} />
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <PrincipleCard
            ...
            items={['UI pura (apresentação)', 'Lógica de negócio (hooks/services)', 'Acesso a dados (API layer)', 'Estado global (stores)']}
          />
          <PrincipleCard
            ...
            items={['Compound Components', 'Render Props quando necessário', 'Custom Hooks para lógica', 'HOCs apenas como último recurso']}
          />
          <PrincipleCard
            icon={<IconShield size={20} />}
            title="Type Safety End-to-End"
            description="TypeScript estrito do backend ao componente. Sem 'any', sem 'as' desnecessário."
            items={['strict: true no tsconfig', 'Zod para validação runtime', 'Tipos gerados da API', 'Branded types para IDs']}
          />
          <PrincipleCard
            icon={<IconBolt size={20} />}
            title="Performance by Default"
            description="Performance não é otimização posterior — é decisão arquitetural desde o dia zero."
            items={['Code splitting por rota', 'Lazy loading de componentes', 'Virtualização de listas', 'Bundle analysis no CI']}
          />
          <PrincipleCard
            icon={<IconBox size={20} />}
            title="Boundaries Explícitos"
            description="Módulos com APIs públicas claras. O que não é exportado, não existe."
            items={['Barrel files (index.ts)', 'Encapsulamento de módulos', 'Dependency inversion', 'Feature flags como boundary']}
          />
          <PrincipleCard
            ...
            items={['Dependency injection', 'Pure functions first', 'Hooks testáveis isolados', 'MSW para mocks de API']}
          />
        </div>
      </PlaybookSection>
...
└── styles/                 # Global styles, tokens`}
        ...
        <DiagramBlock title="Dependency Flow">
          <div className="flex flex-wrap items-center justify-center gap-3 font-mono text-sm">
            <div className="rounded border border-border bg-secondary px-4 py-2 text-foreground">Pages</div>
            <span className="text-muted-foreground">→</span>
            <div className="rounded border border-glow bg-secondary px-4 py-2 text-primary">Features</div>
            <span className="text-muted-foreground">→</span>
            <div className="rounded border border-border bg-secondary px-4 py-2 text-foreground">Shared</div>
            <span className="text-muted-foreground">→</span>
            <div className="rounded border border-border bg-secondary px-4 py-2 text-muted-foreground">Libs</div>
          </div>
          <p className="mt-4 text-center font-mono text-xs text-muted-foreground">
            Dependências sempre fluem para baixo. Features nunca importam de outras features.
          </p>
        </DiagramBlock>
        <div className="mt-8 rounded-lg border border-border bg-card p-6">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li className="flex items-start gap-2">
              <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
              Barrel files definem a API pública. Imports internos são proibidos.
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
              Máximo 1 nível de nesting em features. Se precisa de sub-features, é uma nova feature.
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="py-3 pr-4 text-left font-mono text-xs uppercase tracking-wider text-muted-foreground">Tipo</th>
                <th className="py-3 pr-4 text-left font-mono text-xs uppercase tracking-wider text-muted-foreground">Ferramenta</th>
                <th className="py-3 text-left font-mono text-xs uppercase tracking-wider text-muted-foreground">Exemplo</th>
              </tr>
            </thead>
            <tbody className="text-secondary-foreground">
              <tr className="border-b border-border">
                <td className="py-3 pr-4 font-medium text-foreground">Server State</td>
                <td className="py-3 pr-4">
                  <code className="rounded bg-secondary px-2 py-0.5 font-mono text-xs text-primary">TanStack Query</code>
                </td>
                <td className="py-3 text-muted-foreground">Dados da API, cache, revalidação</td>
              </tr>
              <tr className="border-b border-border">
                <td className="py-3 pr-4 font-medium text-foreground">Client Global</td>
                <td className="py-3 pr-4">
                  <code className="rounded bg-secondary px-2 py-0.5 font-mono text-xs text-primary">Zustand</code>
                </td>
                <td className="py-3 text-muted-foreground">Theme, sidebar, user preferences</td>
              </tr>
              <tr className="border-b border-border">
                <td className="py-3 pr-4 font-medium text-foreground">Form State</td>
                <td className="py-3 pr-4">
                  <code className="rounded bg-secondary px-2 py-0.5 font-mono text-xs text-primary">React Hook Form + Zod</code>
                </td>
                <td className="py-3 text-muted-foreground">Validação, dirty tracking, submit</td>
              </tr>
              <tr className="border-b border-border">
                <td className="py-3 pr-4 font-medium text-foreground">URL State</td>
                <td className="py-3 pr-4">
                  <code className="rounded bg-secondary px-2 py-0.5 font-mono text-xs text-primary">nuqs / searchParams</code>
                </td>
                <td className="py-3 text-muted-foreground">Filtros, paginação, tabs ativas</td>
              </tr>
              <tr>
                <td className="py-3 pr-4 font-medium text-foreground">UI Local</td>
                <td className="py-3 pr-4">
                  <code className="rounded bg-secondary px-2 py-0.5 font-mono text-xs text-primary">useState / useReducer</code>
                </td>
                <td className="py-3 text-muted-foreground">Modals, dropdowns, toggles</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <PrincipleCard
            ...
            items={['Semantic tokens (--color-text-primary)', 'Component tokens (--button-bg)', 'Nunca hardcode valores', 'Suporte a theming via tokens']}
          />
          <PrincipleCard
            icon={<IconBox size={20} />}
            title="Component API Design"
            description="APIs consistentes, previsíveis e compostas. Variantes via CVA."
            items={['Props tipadas com discriminated unions', 'Variants com class-variance-authority', 'Slots pattern para customização', 'Forwarded refs sempre']}
          />
        </div>
        <CodeBlock
      ...
      >
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-lg border border-border bg-card p-6 text-center">
            <IconGauge className="mx-auto mb-3 h-8 w-8 text-primary" />
            <div className="font-mono text-2xl font-bold text-foreground">{'< 200kb'}</div>
            <div className="mt-1 text-xs text-muted-foreground">JS Bundle (gzipped)</div>
          </div>
          <div className="rounded-lg border border-border bg-card p-6 text-center">
            <IconDeviceDesktop className="mx-auto mb-3 h-8 w-8 text-primary" />
            <div className="font-mono text-2xl font-bold text-foreground">{'< 2.5s'}</div>
            <div className="mt-1 text-xs text-muted-foreground">LCP Target</div>
          </div>
          <div className="rounded-lg border border-border bg-card p-6 text-center">
            <IconChartBar className="mx-auto mb-3 h-8 w-8 text-primary" />
      ...
      >
        <DiagramBlock title="Testing Trophy">
          <div className="flex flex-col items-center gap-2 font-mono text-sm">
            <div className="rounded border border-border bg-secondary px-6 py-2 text-muted-foreground">E2E (Playwright) — 10%</div>
            <div className="rounded border border-glow bg-secondary px-10 py-2 text-primary">Integration (Testing Library) — 50%</div>
            <div className="rounded border border-border bg-secondary px-8 py-2 text-foreground">Unit (Vitest) — 30%</div>
            <div className="rounded border border-border bg-secondary px-4 py-2 text-muted-foreground">Static (TypeScript + ESLint) — 10%</div>
          </div>
        </DiagramBlock>
        <CodeBlock
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <PrincipleCard
            ...
            items={['Sentry / Datadog RUM', 'Error Boundaries por feature', 'Breadcrumbs de navegação', 'Custom tags por contexto']}
          />
          <PrincipleCard
            icon={<IconChartBar size={20} />}
            title="Web Vitals"
            description="Core Web Vitals monitorados continuamente em produção."
            items={['LCP, FID/INP, CLS', 'Real User Monitoring (RUM)', 'Alertas em regressões', 'Dashboards por rota']}
          />
          <PrincipleCard
            icon={<IconDatabase size={20} />}
            title="Analytics Estruturado"
            description="Eventos tipados com schema validation. Nada de strings soltas."
            items={['Typed event catalog', 'Schema validation (Zod)', 'Automatic page tracking', 'Feature usage metrics']}
          />
        </div>
        <CodeBlock
